...
import { fetchAndWrapAIInstances } from 'components/model-runner-modal/model-runner-dialog';
import { reloadOpenJobAnnotationsAsync } from 'actions/annotation-actions';
...
export const modelsActions = {
    ...
    showRunModelDialog: (taskInstance: any, jobId?: number) => (
...
export function getModelsAsync(query?: ModelsQuery): ThunkAction {
    return async (dispatch): Promise<void> => {
        ...
        try {
            let baseModels: MLModel[] = [];
            let baseCount = 0;
            try {
                const result = await core.lambda.list();
                baseModels = result.models;
            ...
            let instModels: any[] = [];
            try {
                const wrapped = await fetchAndWrapAIInstances();
                instModels = wrapped?.instModels || [];
            } catch (e) {
                instModels = [];
            }

            const mergedBase = baseModels.filter((m) => !((m as any)?._isAIInstance));
            const seen = new Set<string>();
            const deduped: MLModel[] = [];
            for (const m of [...instModels, ...mergedBase]) {
                const mid = (m as any)?.id;
                const key = typeof mid === 'string' || typeof mid === 'number' ? String(mid) : `fallback_${String(m?.name ?? '')}`;
                if (!key || seen.has(key)) continue;
                seen.add(key);
                deduped.push(m as MLModel);
            }
            dispatch(modelsActions.getModelsSuccess(deduped, deduped.length));
        } catch (error) {
...
function onInferenceFinished(taskID: number, dispatch: ThunkDispatch): void {
    dispatch(reloadOpenJobAnnotationsAsync(taskID));
}
...
function listen(
    ...
    if (status === RQStatus.FINISHED) {
        onInferenceFinished(taskID, dispatch);
        return;
    }
...
export function getInferenceStatusAsync(): ThunkAction {
    return async (dispatch, getState): Promise<void> => {
        const dispatchCallback = dispatch as ThunkDispatch;
...
export function startInferenceAsync(
    taskId: number, model: MLModel, body: object, meta?: object, jobId?: number,
): ThunkAction {
    return async (dispatch): Promise<void> => {
        try {
            ...
            const functionRequest = await core.lambda.run(taskId, model, finalBody, jobId);

            listen(functionRequest, dispatch);
