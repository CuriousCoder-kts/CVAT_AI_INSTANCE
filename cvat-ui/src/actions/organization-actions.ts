...
export enum OrganizationActionsTypes {
    ...
    CREATE_ORGANIZATION_MEMBERS = 'CREATE_ORGANIZATION_MEMBERS',
    CREATE_ORGANIZATION_MEMBERS_SUCCESS = 'CREATE_ORGANIZATION_MEMBERS_SUCCESS',
    CREATE_ORGANIZATION_MEMBERS_FAILED = 'CREATE_ORGANIZATION_MEMBERS_FAILED',
...
export const organizationActions = {
    ...
    createOrganizationMembers: () => createAction(OrganizationActionsTypes.CREATE_ORGANIZATION_MEMBERS),
    createOrganizationMembersSuccess: (result: any) => createAction(
        OrganizationActionsTypes.CREATE_ORGANIZATION_MEMBERS_SUCCESS, { result },
    ),
    createOrganizationMembersFailed: (error: any) => createAction(
        OrganizationActionsTypes.CREATE_ORGANIZATION_MEMBERS_FAILED, { error },
    ),
...
export function createOrganizationMembersAsync(
    organization: Organization,
    members: Array<{
        username: string;
        email: string;
    ...
    return async function (dispatch) {
        dispatch(organizationActions.createOrganizationMembers());
        try {
            const result = await organization.createMembers(members as any[]);
            dispatch(organizationActions.createOrganizationMembersSuccess(result));
            onFinish(result);
        } catch (error) {
            dispatch(organizationActions.createOrganizationMembersFailed(error));
            onFinish({ created_count: 0, error_count: 1, results: [], errors: [{ index: 0, error: error instanceof Error ? error.message : String(error) }] });
        }
    };
}
