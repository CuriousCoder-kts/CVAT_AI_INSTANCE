...
export enum AnnotationActionTypes {
    ...
    TRUNCATE_TRACK_FAILED = 'TRUNCATE_TRACK_FAILED',
...
export function reloadOpenJobAnnotationsAsync(taskID: number): ThunkAction {
    return async (dispatch: ThunkDispatch, getState: () => CombinedState): Promise<void> => {
        const jobInstance = getState().annotation.job.instance as Job | null | undefined;
        if (!jobInstance || jobInstance.taskId !== taskID) {
            return;
        }
    ...
    return async (dispatch: ThunkDispatch): Promise<void> => {
        ...
        if (!jobInstance.annotations.hasUnsavedChanges()) {
            notification.info({
                ...
                description: 'Automatic annotation already writes to the server. Save is only needed after you edit boxes.',
                className: 'cvat-save-no-changes',
            });
            return;
        }
...
export function truncateTrackAsync(
    objectState: CombinedState['annotation']['annotations']['states'][0],
    fromFrame: number,
): ThunkAction {
    return async (dispatch: ThunkDispatch): Promise<void> => {
        const { jobInstance } = receiveAnnotationsParameters();
        try {
            await jobInstance.annotations.truncate(objectState, fromFrame);
            dispatch(fetchAnnotationsAsync());
        } catch (error) {
            dispatch({
                type: AnnotationActionTypes.TRUNCATE_TRACK_FAILED,
                payload: {
                    error,
                },
            });
