...
export function restoreSettingsAsync(): ThunkAction {
    return async (dispatch, getState): Promise<void> => {
        ...
        // Classic CVAT: fit image after changing frame. Old clientSettings often
        // stored resetZoom=false (previous default); migrate once so F/next frame
        // resets zoom without asking every annotator to flip the checkbox.
        const resetZoomMigrateKey = 'cvat.product.resetZoomOnFrame.v1';
        if (!localStorage.getItem(resetZoomMigrateKey)) {
            newSettings.player = { ...newSettings.player, resetZoom: true };
            localStorage.setItem(resetZoomMigrateKey, '1');
        }
        ...
        if ('shortcuts' in loadedSettings) {
            ...
            Object.entries(loadedSettings.shortcuts.keyMap).forEach(([key, value]) => {
                if (key in updateKeyMap) {
                    const loaded = (value as { sequences: string[] }).sequences;
                    // Empty arrays are usually conflict-resolver leftovers, not an intentional unbind.
                    // Applying them disabled stock Delete / N for annotators.
                    if (Array.isArray(loaded) && loaded.length) {
                        updateKeyMap[key].sequences = loaded;
                    }
                }
            });
