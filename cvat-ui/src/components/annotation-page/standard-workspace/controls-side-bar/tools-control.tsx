...
export class ToolsControlComponent extends React.PureComponent<Props, State> {
    ...
    private getSupportedTrackers(): MLModel[] {
        ...
        return trackers.filter((tracker: MLModel) => (
            tracker.supportedShapeTypes || [ShapeType.RECTANGLE]
        ).includes(ShapeType.RECTANGLE));
    }
                ...
                for (const [trackerID, trackableObjects] of trackingData.stateful) {
                    ...
                    try {
                        ...
                        for (let i = 0; i < trackableObjects.clientIDs.length; i++) {
                            ...
                            const rawShape = response.shapes[i];
                            ...
                            const pts = rawShape?.points;
                            if (!objectState || !Array.isArray(pts) || pts.length < 4) {
                            ...
                            const shape = trackedRectangleMapper(rawShape);
                        ...
                        if (lostClientIDs.length) {
                            this.setState((prev) => ({
                                trackedShapes: prev.trackedShapes.filter(
                                    (trackedShape: TrackedShape) => !lostClientIDs.includes(trackedShape.clientID),
                                ),
                            }));
                        }
                    } catch (error: any) {
        ...
        return (
            <>
                ...
                <Row justify='start' style={{ marginTop: 8 }}>
                    <Col span={24}>
                        <Text type='secondary' className='cvat-text-color' style={{ fontSize: 12, display: 'block' }}>
                            选标签（如 car）→ 选 Optical Flow (DIS) → Track → 画矩形（生成 Track）→ 按 F 或播放。
                            跟丢会停止；改框后再跟。不要用 Automatic annotation。
                        </Text>
                    </Col>
                </Row>
            </>
            ...
            <>
                ...
                <CustomPopover
                    ...
                    onVisibleChange={(visible: boolean) => {
                        if (visible !== this.state.toolsPopoverVisible) {
