...
import Icon, {
    ...
    FunctionOutlined, VerticalAlignBottomOutlined, ScissorOutlined,
...
function TruncateFromHereItem(props: ItemProps): JSX.Element {
    const { toolProps } = props;
    const { truncateFromHere } = toolProps;
    return (
        <Button
            type='link'
            icon={<ScissorOutlined />}
            onClick={truncateFromHere}
            className='cvat-object-item-menu-truncate-track'
        >
...
export default function ItemMenu(props: Props): MenuProps {
    ...
    enum MenuKeys {
        ...
        TRUNCATE_FROM_HERE = 'truncate_from_here',
        EDIT_MASK = 'edit_mask',
    ...
    if (!locked && objectType === ObjectType.TRACK) {
        items.push({
            key: MenuKeys.TRUNCATE_FROM_HERE,
            label: <TruncateFromHereItem toolProps={props} />,
        });
    }
