...
/** Keep selection while moving toward resize/rotation handles of the active box. */
const ACTIVE_HANDLE_KEEP_PX = 36;
/** Survives minify; used by deploy verify. */
const KEEP_ACTIVE_ROT_MARKER = 'cvat-keep-active-rotation-zone';
...
/** True when cursor is on/near the active rectangle (incl. rotation knob above top). */
function cursorInActiveRectHandleZone(state: ObjectState, x: number, y: number): boolean {
    if (![ShapeType.RECTANGLE, ShapeType.ELLIPSE].includes(state.shapeType as ShapeType)) {
        return false;
    }
    ...
    let xtl = Math.min(pts[0], pts[2]);
    let ytl = Math.min(pts[1], pts[3]);
    let xbr = Math.max(pts[0], pts[2]);
    let ybr = Math.max(pts[1], pts[3]);
    if (state.shapeType === ShapeType.ELLIPSE) {
        const cx0 = pts[0];
        const cy0 = pts[1];
        const rx = Math.abs(pts[2] - pts[0]);
        const ry = Math.abs(pts[3] - pts[1]);
        xtl = cx0 - rx;
        ytl = cy0 - ry;
        xbr = cx0 + rx;
        ybr = cy0 + ry;
    ...
    const cx = (xtl + xbr) / 2;
    const cy = (ytl + ybr) / 2;
    const [rx, ry] = rotatePoint2D(x, y, state.rotation || 0, cx, cy);
    const pad = ACTIVE_HANDLE_KEEP_PX;
    if (rx >= xtl - pad && rx <= xbr + pad && ry >= ytl - pad && ry <= ybr + pad) {
        return true;
    }
    // Rotation knob sits above the top-edge midpoint (see canvasView.refreshRotationPointView).
    const rotX = (xtl + xbr) / 2;
    const rotY = ytl - ACTIVE_HANDLE_KEEP_PX;
    const dRot = Math.hypot(rx - rotX, ry - rotY);
    return dRot <= ACTIVE_HANDLE_KEEP_PX;
}
...
class CanvasWrapperComponent extends React.PureComponent<Props> {
    ...
    private onCanvasMouseDown = (e: MouseEvent): void => {
        ...
        // Selecting a shape must return keyboard focus to the workspace. Ant Design Select
        // (label picker) is an <input>; if it keeps focus, Mousetrap will not fire Delete/N.
        const active = document.activeElement;
        if (
            active instanceof HTMLElement &&
            !canvasInstance.html().contains(active)
        ) {
    ...
    private onCanvasCursorMoved = async (event: any): Promise<void> => {
        ...
        // Moving toward the white rotation knob often crosses another overlapping
        // box (e.g. Rider over Bicycle). Hover would switch selection and remove
        // the active handles before the knob can be clicked — keep the active box.
        if (activatedStateID !== null) {
            const activated = (event.detail.states as ObjectState[]).find(
                (s) => s.clientID === activatedStateID,
            );
            if (
                activated &&
                !activated.outside &&
                !activated.hidden &&
                cursorInActiveRectHandleZone(activated, event.detail.x, event.detail.y)
            ) {
                // Side-effect keeps the marker string in the minified bundle for deploy verify.
                try {
                    (window as any)[KEEP_ACTIVE_ROT_MARKER] = 1;
                } catch (_e) {
                    // ignore
                }
                return;
