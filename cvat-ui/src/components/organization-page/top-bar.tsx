...
function OrganizationTopBar(props: Readonly<Props>): JSX.Element {
    const {
        organizationInstance, userInstance, members, fetchMembers, query,
    ...
    const currentUserMembership = members.find(
        (m: Membership) => m.user && m.user.id === userID,
    );
    const currentUserRole = currentUserMembership ? currentUserMembership.role : null;
    const canCreateMembersDirectly = currentUserRole === 'owner' || currentUserRole === 'maintainer';
    ...
    const onCreate = useCallback((values: Store, onFinish: (r: any) => void) => {
        dispatch(createOrganizationMembersAsync(
            organizationInstance,
            values.members as any[],
            (result) => {
                fetchMembers();
                onFinish(result);
            },
            ...
            <Row justify='space-between'>
                                ...
                                <Dropdown
                                    menu={{
                                        items: [
                                            ...
                                            {
                                                key: MenuActions.SET_BAILIAN,
                                                label: <Link to='/organization/bailian'>Bailian settings</Link>,
                                            },
                ...
                <Col span={12} className='cvat-organization-top-bar-buttons-block'>
                    <Space align='end'>
                        ...
                        {canCreateMembersDirectly ? (
                            <Button
                                className='cvat-create-org-members-button'
                                type='primary'
