...
function validateParsedAttribute(attr: SerializedAttribute): void {
    ...
    // number attrs store [min, max, step] — min===step is valid (e.g. ["1","10","1"]).
    // checkbox/text do not use an enum values list either.
    const inputType = (attr.input_type ?? '').toLowerCase();
...
export function validateParsedLabel(label: SerializedLabel): void {
    ...
    // Older exports omit type; treat missing/empty as "any" (same as Constructor save).
    if (label.type == null || !String(label.type).trim()) {
