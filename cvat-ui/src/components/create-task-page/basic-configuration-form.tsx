...
export default class BasicConfigurationForm extends React.PureComponent<Props> {
    ...
    componentDidUpdate(prevProps: Props): void {
        const { many, onChange } = this.props;
        if (!many || prevProps.many) {
            return;
        }

        const current = this.formRef.current?.getFieldValue('name') as string | undefined;
        if (current && (current.includes('{{file_name}}') || current.includes('{{index}}'))) {
            return;
        }

        this.formRef.current?.setFieldsValue({ name: '{{file_name}}' });
        onChange({ name: '{{file_name}}' });
    }
