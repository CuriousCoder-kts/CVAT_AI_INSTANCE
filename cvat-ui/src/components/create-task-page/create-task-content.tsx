...
import {
    ...
    getFileNameFromPath,
    groupLocalFilesByTopFolder,
    prepareLocalUploadFiles,
    isImageFileName,
...
const UploadFileHints = {
    one: 'You can upload an archive with images, a video, multiple images, or a parent folder of image folders',
...
class CreateTaskContent extends React.PureComponent<Props & RouteComponentProps, State> {
    ...
    private handleUploadLocalFiles = (uploadedFiles: File[]): void => {
        ...
        let uploadFileErrorMessage = '';
        let filesToStore = uploadedFiles;
        const prepared = prepareLocalUploadFiles(uploadedFiles);

        if (prepared.fromDirectory && prepared.files.length) {
            filesToStore = prepared.files;
        } else if (prepared.fromDirectory && !many) {
            uploadFileErrorMessage = 'No images found in the selected folder.';
        } else {
            ...
            if (!many && excludedManifests.length > 1) {
                uploadFileErrorMessage = excludedManifests.every((it) => (
                    getFileContentType(it) === SupportedShareTypes.IMAGE || isImageFileName(it.name)
                )) ? '' : UploadFileErrorMessages.one;
            } else if (many) {
        ...
        if (!uploadFileErrorMessage) {
            this.setState({
                ...
                if (this.getLocalFolderGroups()) {
                    const { basic } = this.state;
                    if (!basic.name.includes('{{file_name}}') && !basic.name.includes('{{index}}')) {
                        this.basicConfigurationComponent.current?.setName('{{file_name}}');
                    }
                }
            });
        }
    ...
    private getLocalFolderGroups = (): LocalFolderGroup[] | null => {
        const { files, activeFileManagerTab } = this.state;
        if (activeFileManagerTab !== 'local') {
            return null;
        }
        const grouped = groupLocalFilesByTopFolder(files.local);
        if (grouped.mode !== 'folders') {
            return null;
        }
        return grouped.groups;
    };

    private usesMultiTaskFlow = (): boolean => {
        const { many } = this.props;
        return many || this.getLocalFolderGroups() !== null;
    };
    ...
    private addMultiTasks = async (): Promise<void> => new Promise((resolve) => {
        ...
        const folderGroups = this.getLocalFolderGroups();
        const files: (File | string)[] = allFiles[activeFileManagerTab];

        const buildTask = (
            index: number,
            taskFiles: (File | string)[],
        ): State['multiTasks'][number] => ({
            projectId,
            basic: {
            ...
            files: {
                ...defaultState.files,
                [activeFileManagerTab]: taskFiles,
            },
            activeFileManagerTab,
            cloudStorageId,
            status: 'pending',
        ...
        this.setState({
            multiTasks: folderGroups ?
                folderGroups.map((group, index) => buildTask(index, group.files)) :
                files.map((file, index) => buildTask(index, [file])),
        }, resolve);
    });
    ...
    private getTaskName = (indexFile: number, fileManagerTabName: TabName, defaultFileName = ''): string => {
        ...
        const { files } = this.state;
        const folderGroups = this.getLocalFolderGroups();
        const useTemplate = many || !!folderGroups;
        const file = files[fileManagerTabName][indexFile];
        let fileName = defaultFileName;
        if (folderGroups) {
            fileName = folderGroups[indexFile]?.name || defaultFileName;
        } else {
    ...
    private renderBasicBlock(): JSX.Element {
        const { many } = this.props;
        const folderGroups = this.getLocalFolderGroups();
        const usesTemplate = many || !!folderGroups;
        const exampleMultiTaskName = usesTemplate ?
            this.getTaskName(0, 'local', folderGroups?.[0]?.name || 'fileName.mp4') :
            '';
    ...
    private renderFilesBlock(): JSX.Element {
        const { many } = this.props;
        const { uploadFileErrorMessage, files } = this.state;
        const folderGroups = this.getLocalFolderGroups();
        const folderImageCount = folderGroups?.reduce((acc, group) => acc + group.files.length, 0) || 0;
        const pickedDirectory = files.local.some((file) => getWebkitRelativePath(file).includes('/'));
        const singleFolderImageCount = files.local.filter((file) => isImageFileName(file.name)).length;
        ...
        return (
            <>
                ...
                { folderGroups ? (
                    <Col span={24}>
                        <Alert
                            className='cvat-create-task-folder-batch-alert'
                            type='info'
                            showIcon
                            message={
                                `${folderGroups.length} folders, ${folderImageCount} images. ` +
                                'Each folder will be created as a separate task. ' +
                                'Attach a Project so every task inherits the same labels.'
                            }
                        />
                    </Col>
                ) : null }
                { !folderGroups && pickedDirectory ? (
                    <Col span={24}>
                        <Alert
                            className='cvat-create-task-single-folder-alert'
                            type='warning'
                            showIcon
                            message={
                                `Only 1 image folder was found (${singleFolderImageCount} images), ` +
                                'so Submit & Open will create a single task. ' +
                                'To create one task per folder, click Select a folder and pick the ' +
                                'directory that contains those folders (one level above them).'
                            }
                        />
                    </Col>
                ) : null }
    ...
    private renderFooterMultiTasks(): JSX.Element {
        ...
        const folderGroups = this.getLocalFolderGroups();
        const taskCount = folderGroups ? folderGroups.length : currentFiles.length;
        const countPending = items.filter((item) => item.status === 'pending').length;
        const countAll = items.length;

        if ((loading || countPending !== countAll) && taskCount) {
            return (
                <MultiTasksProgress
                    tasks={items}
        ...
        return (
            <Row justify='start' align='middle' className='cvat-create-task-content'>
                ...
                <Col span={24} className='cvat-create-task-content-footer'>
                    {this.usesMultiTaskFlow() ? this.renderFooterMultiTasks() : this.renderFooterSingleTask() }
                </Col>
            </Row>
        );
    }
