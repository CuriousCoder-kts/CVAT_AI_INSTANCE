...
import { getObjectStateColor, readDetectorTrackId } from 'components/annotation-page/standard-workspace/objects-side-bar/shared';
...
interface DispatchToProps {
    ...
    truncateTrack: (objectState: ObjectState, fromFrame: number) => void;
...
function mapStateToProps(state: CombinedState, own: OwnProps): StateToProps {
    ...
    const objectState = index >= 0 ? states[index] : null;
    const labelId = objectState?.label?.id as number | undefined;

    return {
        objectState,
        attributes: (labelId != null ? jobAttributes[labelId] : undefined) || [],
        labels,
        ready,
        activeControl,
        colorBy,
...
function mapDispatchToProps(dispatch: any): DispatchToProps {
    return {
        ...
        truncateTrack(objectState: ObjectState, fromFrame: number): void {
            dispatch(truncateTrackAsync(objectState, fromFrame));
        },
...
class ObjectItemContainer extends React.PureComponent<Props, State> {
    ...
    private truncateFromHere = (): void => {
        const { objectState, frameNumber, truncateTrack } = this.props;
        if (objectState.objectType !== ObjectType.TRACK) {
            return;
        }
        const firstKeyframe = objectState.keyframes?.first ?? frameNumber;
        const removesAll = frameNumber <= firstKeyframe;
        Modal.confirm({
            title: removesAll ? 'Remove entire track' : `Delete track from frame ${frameNumber}`,
            content: removesAll ?
                `Frame ${frameNumber} is at or before the first keyframe, so the whole track will be removed.` :
                `Keep this object on frames before ${frameNumber}. Remove it from frame ${frameNumber} to the end of the job. Grouped shapes (for example VanCar + keypoints + Rear) are truncated together.`,
            okText: removesAll ? 'Remove track' : 'Delete from this frame',
            okType: 'danger',
            cancelText: 'Cancel',
            className: 'cvat-modal-confirm-truncate-track',
            onOk: () => truncateTrack(objectState, frameNumber),
        });
    };
        ...
        return (
            <>
                <ObjectStateItemComponent
                    ...
                    elements={elements}
                    trackId={readDetectorTrackId(
                        attributes,
                        objectState.attributes as Record<number, string> | undefined,
                    )}
                    ...
                    remove={this.remove}
                    truncateFromHere={this.truncateFromHere}
                    copy={this.copy}
