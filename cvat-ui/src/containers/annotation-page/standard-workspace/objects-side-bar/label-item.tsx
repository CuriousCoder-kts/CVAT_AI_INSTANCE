...
function mapStateToProps(state: CombinedState, own: OwnProps): StateToProps {
    ...
    const label = labels.find((_label: any) => _label.id === own.labelID);

    return {
        label,
        // Guard: annotations may reference a deleted/missing label id.
        labelColor: label?.color || '#000000',
        labelName: label?.name || `Unknown label #${own.labelID}`,
        objectStates,
        jobInstance,
        frameNumber,
    };
