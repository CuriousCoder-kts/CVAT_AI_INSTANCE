...
from cvat.apps.events.stats import query_annotation_statistics
...
class EventsViewSet(viewsets.ViewSet):
    ...
    @extend_schema(
        summary="Get per-user annotation statistics",
        methods=["GET"],
        description=(
            "Aggregates ClickHouse create:shapes/tags/tracks events and working time "
            "per user, split into manual / AI (source=auto) / imported (source=file). "
            "Organization owner/maintainer/supervisor see all members; "
        ...
        parameters=[
            OpenApiParameter(
                ...
                type=OpenApiTypes.DATETIME,
                required=False,
                description="UTC start datetime. Default is 30 days ago.",
            ),
            OpenApiParameter(
                "to",
                location=OpenApiParameter.QUERY,
                type=OpenApiTypes.DATETIME,
                required=False,
                description="UTC end datetime. Default is now.",
            ),
            OpenApiParameter(
                "project_id",
                location=OpenApiParameter.QUERY,
        ...
        responses={"200": OpenApiResponse(description="Per-user and per-task annotation statistics")},
    )
    @action(detail=False, methods=["GET"], url_path="statistics")
    def statistics(self, request: ExtendedRequest):
        self.check_permissions(request)
        perm = EventsPermission.create_base_perm(
            request, self, EventsPermission.Scopes.VIEW_STATISTICS, None
        )
        filter_params = perm.filter(request.query_params)
        return Response(query_annotation_statistics(filter_params))
