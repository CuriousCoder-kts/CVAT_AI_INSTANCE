...
class Mpeg4ChunkWriter(IChunkWriter):
    ...
    def __init__(self, *, quality: int, dimension: DimensionType) -> None:
        ...
        try:
            ...
            # Do not set qmin/qmax: libopenh264 avcodec_send_frame() fails on real
            # (non-flat) 1080p frames whenever qmin is set, including qmin=15.
            self._codec_opts = {
    ...
    def _encode_images(
        ...
        dst_w = int(stream.width)
        dst_h = int(stream.height)
        dst_fmt = stream.pix_fmt or "yuv420p"
        for frame, _ in images:
            if not isinstance(frame, av.VideoFrame):
                raise TypeError(f"Mpeg4ChunkWriter expected av.VideoFrame, got {type(frame)}")
            # Always restamp via bgr24. Real .ts frames are yuvj420p (JPEG range /
            # colorspace 5); libopenh264 avcodec_send_frame() rejects those even
            # after reformat(yuv420p). A synthetic bgr24 frame encodes; a decoded
            # camera frame does not, unless we drop the source color metadata.
            arr = frame.to_ndarray(format="bgr24")
            if arr.shape[1] != dst_w or arr.shape[0] != dst_h:
                import cv2

                arr = cv2.resize(arr, (dst_w, dst_h), interpolation=cv2.INTER_AREA)
            frame = av.VideoFrame.from_ndarray(arr, format="bgr24")
            if dst_fmt != "bgr24":
                frame = frame.reformat(format=dst_fmt, width=dst_w, height=dst_h)
            frame.pts = None
