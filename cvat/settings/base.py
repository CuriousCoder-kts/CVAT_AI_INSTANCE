...
REST_FRAMEWORK = {
    "DEFAULT_PARSER_CLASSES": [
        "cvat.apps.engine.parsers.CVATJsonParser",
...
# Corrections / Acceptance access (OPA + /api/acceptance/access-policy).
# Primary gate: Profile.has_acceptance_access (AND IAM admin in OPA).
# Optional expansion paths (env):
# CVAT_ACCEPTANCE_ORG_ROLES: comma-separated org roles also allowed, e.g. "owner"
#   or "owner,maintainer". Empty (default) = no org-role path.
# CVAT_ACCEPTANCE_ALLOW_TASK_STAFF: if true, task/project owner/assignee also allowed.
ACCEPTANCE_ORG_ROLES = tuple(
    role.strip().lower()
    for role in os.getenv("CVAT_ACCEPTANCE_ORG_ROLES", "").split(",")
    if role.strip()
)
ACCEPTANCE_ALLOW_TASK_STAFF = os.getenv(
    "CVAT_ACCEPTANCE_ALLOW_TASK_STAFF", "false"
