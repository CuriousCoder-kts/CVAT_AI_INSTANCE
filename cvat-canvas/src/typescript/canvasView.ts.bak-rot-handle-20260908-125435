...
export class CanvasViewImpl implements CanvasView, Listener {
    ...
    private onKeyDown = (e: KeyboardEvent): void => {
        ...
        if (code.includes('control') || e.key === 'Control') {
            this.ctrlPressed = true;
        }
    };

    // Screenshot tools (Ctrl+Alt+A / WeChat / DingTalk) steal focus and swallow keyup,
    // which otherwise latches ctrlPressed and disables mouse-wheel zoom until reload.
    private resetStuckModifiers = (): void => {
        this.ctrlPressed = false;
        this.snapToAngleResize = consts.SNAP_TO_ANGLE_RESIZE_DEFAULT;
    };

    private onWindowBlur = (): void => {
        this.resetStuckModifiers();
    };
    ...
    public constructor(model: CanvasModel & Master, controller: CanvasController) {
        ...
        this.canvas.setAttribute('data-cvat-unstick-ctrl', '1');
        ...
        this.canvas.addEventListener('wheel', (event): void => {
            if (this.ctrlPressed) {
                // Physical Ctrl is tracked separately from event.ctrlKey so trackpad pinch-zoom
                // (which sets ctrlKey without a real Control key) still zooms the canvas.
                // If Control is no longer reported, keyup was lost (screenshot overlay, etc.).
                if (!event.ctrlKey) {
                    this.resetStuckModifiers();
                } else {
                    return;
        ...
        this.canvas.addEventListener('mousemove', (e): void => {
            ...
            // Do not re-run shape hover selection while over active resize/rotate
            // handles — otherwise an overlapping box steals activation and the
            // white rotation point disappears before it can be clicked.
            const target = e.target as Element | null;
            if (
                target &&
                typeof (target as Element).closest === 'function' &&
                (target as Element).closest(
                    '.svg_select_points, .svg_select_points_rot, .cvat_canvas_selected_point',
                )
            ) {
                return;
            }
        ...
        for (const state of states) {
            ...
            const trackIdChanged = ((): boolean => {
                const names = Object.fromEntries(
                    (state.label?.attributes || []).map((attr: { id: number; name: string }) => [attr.id, attr.name]),
                );
                return Object.keys(state.attributes || {}).some((attrID: string) => {
                    const name = String(names[attrID] || '').toLowerCase();
                    return (
                        (name === 'track_id' || name === 'object_id' || name === 'instance_id') &&
                        state.attributes[attrID] !== drawnState.attributes[+attrID]
                    );
                });
            })();
    ...
    private deactivateShape(): void {
        if (this.activeElement.clientID) {
            ...
            Object.values(this.svgShapes).forEach((other) => {
                if (!other) {
                    return;
                }
                try {
                    other.attr('pointer-events', null);
                } catch (_e) {
                    // ignore
                }
            });
    ...
    private activateShape(clientID: number): void {
        ...
        if (state.shapeType !== 'points') {
            ...
            // selectize nests handles as siblings; moving only the shape leaves
            // resize/rotation handles under overlapping shapes (e.g. Rider over
            // Bicycle's white rotation point). Keep the handle group on top.
            const selectHandler = shape.remember('_selectHandler');
            if (selectHandler?.nested?.node) {
                selectHandler.nested.node.setAttribute('data-cvat-select-handles-on-top', '1');
                this.content.append(selectHandler.nested.node);
            }
        }

        // Let pointer events pass through other shapes so the active rotation
        // knob remains reachable when another box overlaps it.
        Object.entries(this.svgShapes).forEach(([id, other]) => {
            if (!other || +id === clientID) {
                return;
            }
            try {
                other.attr('pointer-events', 'none');
            } catch (_e) {
                // ignore non-SVG entries
            }
        });
        // marker: cvat-keep-active-rotation-zone
    ...
    private addText(state: any, options: { textContent?: string, isSkeletonElement?: boolean } = {}): SVG.Text {
        ...
        const detectorTrackId = ((): string => {
            const specs = state?.label?.attributes;
            const values = state?.attributes;
            if (!Array.isArray(specs) || !values) {
                return '';
            }
            const spec = specs.find((item: { id?: number; name?: string }) => {
                const name = String(item?.name || '').toLowerCase();
                return name === 'track_id' || name === 'object_id' || name === 'instance_id';
            });
        ...
        this.svgTexts[state.clientID] = this.adoptedText
            .text((block): void => {
                ...
                if (detectorTrackId) {
                    block
                        .tspan(`tid=${detectorTrackId}`)
                        .attr({
                            dy: '1.25em',
                            x: 0,
                        })
                        .addClass('cvat_canvas_text_track_id')
                        .style({
                            'text-transform': 'none',
                        });
                }
                ...
                if (withAttr) {
                    Object.keys(attributes).filter((attrID: string) => {
                        const name = String(attrNames[attrID] || '').toLowerCase();
                        return name !== 'track_id' && name !== 'object_id' && name !== 'instance_id';
                    }).forEach((attrID: string, idx: number) => {
