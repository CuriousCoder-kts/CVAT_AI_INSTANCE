...
function build(): CVATCore {
    const cvat: CVATCore = {
        ...
        lambda: {
            ...
            async run(task, model, args, jobID) {
                const result = await PluginRegistry.apiWrapper(cvat.lambda.run, task, model, args, jobID);
                return result;
            },
        ...
        analytics: {
            events: {
                ...
                async statistics(filter = {}) {
                    const result = await PluginRegistry.apiWrapper(cvat.analytics.events.statistics, filter);
                    return result;
                },
            },
            quality: {
