...
class LambdaManager {
    ...
    async run(taskID: number, model: MLModel, args: any, jobID?: number): Promise<SerializedFunctionRequest> {
        if (!Number.isInteger(taskID) || taskID < 0) {
            throw new ArgumentError(`Argument taskID must be a positive integer. Got "${taskID}"`);
        }

        const anyModel = model as any;
        const isAIInstanceWrapper = !!anyModel?._isAIInstance;

        if (!isAIInstanceWrapper && !(model instanceof MLModel)) {
        ...
        const body: Record<string, any> = {
            ...
            task: taskID,
            function: model.id,
            ...(isAIInstanceWrapper && anyModel._aiInstanceSlug ? {
                requested_instance_slug: anyModel._aiInstanceSlug,
                ...(anyModel._aiInstanceID ? { ai_instance_id: anyModel._aiInstanceID } : {}),
            } : {}),
        };
