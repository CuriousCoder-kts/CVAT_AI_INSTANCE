...
export interface SerializedBailianSettings {
    api_url: string;
    model: string;
    has_api_key: boolean;
    updated_date: string;
    updated_by: SerializedUser | null;
}

export type AIFeatureKind = 'object_detector' | 'image_descriptor' | 'object_painter'
    | 'image_caption' | 'intention_estimation' | 'pose_estimation' | 'object_tracker';
export type AIProviderKind = 'bailian' | 'local' | 'openai' | 'custom';

export interface SerializedAIFunctionInstance {
    id: number;
    slug: string;
    name: string;
    feature_kind: AIFeatureKind;
    provider: AIProviderKind;
    is_enabled: boolean;
    is_default: boolean;
    config: Record<string, any>;
    has_sensitive_config: Record<string, boolean>;
    nuclio_function_id: string;
    created_date: string;
    updated_date: string;
    updated_by: SerializedUser | null;
...
export interface SerializedAIFunctionInstanceWrite {
    slug?: string;
    name?: string;
    feature_kind?: AIFeatureKind;
    provider?: AIProviderKind;
...
export interface APIAnnotationStatisticsFilter {
    from?: string;
    to?: string;
    project_id?: number;
    task_id?: number;
...
export interface SerializedAnnotationStatisticsUser {
    id: number;
    name: string;
    shapes: number;
    tags: number;
...
export interface SerializedAnnotationStatisticsTask {
    id: number;
    name: string;
    shapes: number;
    tags: number;
...
export interface SerializedAnnotationStatistics {
    available: boolean;
    message?: string;
    from: string;
    to: string;
    users: SerializedAnnotationStatisticsUser[];
    tasks?: SerializedAnnotationStatisticsTask[];
    daily: { date: string; count: number; manual?: number; ai?: number; imported?: number; other?: number }[];
    totals: {
        annotations: number;
        manual?: number;
        ai?: number;
