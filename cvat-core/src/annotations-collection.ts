...
import { copyShape } from './annotations-objects/utils';
import type { TrackedShape } from './annotations-objects/types';
...
export default class Collection {
    ...
    private cloneTrackedShapes(shapes: Record<number, TrackedShape>): Record<number, TrackedShape> {
        const cloned: Record<number, TrackedShape> = {};
        for (const [frame, shape] of Object.entries(shapes)) {
            cloned[+frame] = {
                ...
                attributes: new Map(shape.attributes),
            };
        }
        return cloned;
    }

    private truncatedShapesFrom(track: Track, fromFrame: number): Record<number, TrackedShape> | 'delete' {
        const frames = Object.keys(track.shapes).map((frame) => +frame).sort((a, b) => a - b);
        if (!frames.length || fromFrame <= frames[0]) {
            return 'delete';
        }

        const next: Record<number, TrackedShape> = {};
        for (const frame of frames) {
            if (frame < fromFrame) {
                next[frame] = this.cloneTrackedShapes({ [frame]: track.shapes[frame] })[frame];
            }
        }

        let position;
        try {
            position = track.get(fromFrame);
        } catch (error) {
            if (error instanceof InterpolationNotPossibleError) {
                return 'delete';
            }
        ...
        const existing = track.shapes[fromFrame];
        next[fromFrame] = copyShape(position, {
            outside: true,
            serverId: existing?.serverId,
            attributes: existing ? new Map(existing.attributes) : new Map(),
        });
        return next;
    }

    private addTrackWithElements(track: Track, tracks: Track[]): void {
        if (track.removed) {
            return;
        }
        if (!tracks.some((item) => item.clientID === track.clientID)) {
            tracks.push(track);
        }
        if (track instanceof SkeletonTrack) {
            for (const element of track.elements) {
                if (!element.removed && !tracks.some((item) => item.clientID === element.clientID)) {
                    tracks.push(element);
                }
            }
        }
    ...
    // Keep keyframes before fromFrame; end the track at fromFrame (outside) and drop later keys.
    public truncate(objectStates: ObjectState[] | ObjectState, fromFrame: number): void {
        const states = (Array.isArray(objectStates) ? objectStates : [objectStates])
            .filter((state, index, all) => all.findIndex((item) => item.clientID === state.clientID) === index);
        checkObjectType('frame', fromFrame, 'integer', null);
        if (!states.length) {
            return;
        }
        ...
        for (const objectState of states) {
            ...
            this.addTrackWithElements(object as Track, tracks);
        }

        const groupIDs = new Set(
            tracks.filter((track) => track.group && this.tracks.includes(track)).map((track) => track.group),
        );
        if (groupIDs.size) {
            for (const track of this.tracks) {
                if (groupIDs.has(track.group)) {
                    this.addTrackWithElements(track, tracks);
                }
            }
        }

        if (!tracks.length) {
            throw new ArgumentError('Selected objects are not tracks');
        }

        if (tracks.some((track) => track.lock)) {
            throw new ArgumentError('The object is locked');
        }

        const snapshots = tracks.map((track) => ({
            track,
            shapes: this.cloneTrackedShapes(track.shapes),
            removed: track.removed,
        }));

        const redoSnapshots: typeof snapshots = [];
        for (const track of tracks) {
            const truncated = this.truncatedShapesFrom(track, fromFrame);
            if (truncated === 'delete') {
                track.removed = true;
            } else {
                track.shapes = truncated;
            }
        ...
        this.history.do(
            HistoryActions.TRUNCATED_TRACK,
            () => {
                for (const snapshot of snapshots) {
                    snapshot.track.shapes = this.cloneTrackedShapes(snapshot.shapes);
                    snapshot.track.removed = snapshot.removed;
                    snapshot.track.updated = Date.now();
                }
            },
            () => {
                for (const snapshot of redoSnapshots) {
                    snapshot.track.shapes = this.cloneTrackedShapes(snapshot.shapes);
                    snapshot.track.removed = snapshot.removed;
                    snapshot.track.updated = Date.now();
                }
    ...
    public select(objectStates: ObjectState[], x: number, y: number): {
        ...
        // Rectangle/ellipse distance is "distance to nearest edge" while inside.
        // Near a box border that value is tiny and beats a nearby keypoint, so the
        // box steals hover/click and the white point "disappears". Prefer point-like
        // shapes when the cursor is close enough to them.
        // marker: cvat-prefer-nearby-points
        const POINT_LIKE_PREFER_PX = 20;

        let minimumDistance = null;
        let minimumState = null;
        let pointLikeDistance = null;
        let pointLikeState = null;
        for (const state of objectStates) {
            ...
            if (
                distance !== null &&
                (state.shapeType === ShapeType.POINTS || state.shapeType === ShapeType.POLYLINE) &&
                distance <= POINT_LIKE_PREFER_PX &&
                (pointLikeDistance === null || distance < pointLikeDistance)
            ) {
                pointLikeDistance = distance;
                pointLikeState = state;
            }
        }

        if (pointLikeState !== null) {
            return {
                state: pointLikeState,
                distance: pointLikeDistance,
            };
        }
