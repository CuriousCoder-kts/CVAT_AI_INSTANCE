...
import {
    ...
    SerializedApiToken, APIApiTokensFilter, SerializedBailianSettings,
    APIAnnotationStatisticsFilter, SerializedAnnotationStatistics,
...
async function getAnnotationStatistics(
    params: APIAnnotationStatisticsFilter,
): Promise<SerializedAnnotationStatistics> {
    const { backendAPI } = config;

    try {
        const { data } = await Axios.get(`${backendAPI}/events/statistics`, { params });
        return data;
    } catch (errorData) {
        throw generateError(errorData);
    }
...
async function getOrganizationBailianSettings(organizationID: number): Promise<SerializedBailianSettings> {
    const { backendAPI } = config;

    let response = null;
    try {
        response = await Axios.get(`${backendAPI}/organizations/${organizationID}/bailian-settings`);
    } catch (errorData) {
        throw generateError(errorData);
    }
...
async function updateOrganizationBailianSettings(
    organizationID: number, data: { api_url?: string; model?: string; api_key?: string | null },
): Promise<SerializedBailianSettings> {
    const { backendAPI } = config;

    let response = null;
    try {
        response = await Axios.patch(`${backendAPI}/organizations/${organizationID}/bailian-settings`, data);
    } catch (errorData) {
        throw generateError(errorData);
    }
...
async function listOrganizationAIFunctionInstances(
    organizationID: number,
    filters: { feature_kind?: string; provider?: string; is_enabled?: boolean } = {},
): Promise<SerializedAIFunctionInstance[]> {
    const { backendAPI } = config;
    let response = null;
    try {
        response = await Axios.get(`${backendAPI}/organizations/${organizationID}/ai-function-instances`, {
            params: filters,
        });
    } catch (errorData) {
        throw generateError(errorData);
    ...
    let results: SerializedAIFunctionInstance[];
    let count: number | undefined;
    let next: string | null | undefined;
    if (data && typeof data === 'object' && Array.isArray(data.results)) {
        results = data.results as SerializedAIFunctionInstance[];
        count = data.count;
        next = data.next;
    } else if (Array.isArray(data)) {
        results = data as SerializedAIFunctionInstance[];
        count = data.length;
    } else {
        results = [];
        count = 0;
    ...
    if (typeof count !== 'undefined') {
        Object.defineProperty(results, 'count', { value: count, writable: false, enumerable: false });
    }
    if (typeof next !== 'undefined') {
        Object.defineProperty(results, 'next', { value: next, writable: false, enumerable: false });
    }
    return results;
}

async function getOrganizationAIFunctionInstance(
    organizationID: number,
    slug: string,
): Promise<SerializedAIFunctionInstance> {
    const { backendAPI } = config;
    let response = null;
    try {
        response = await Axios.get(`${backendAPI}/organizations/${organizationID}/ai-function-instances/${slug}`);
    } catch (errorData) {
        throw generateError(errorData);
    }
    return response.data;
...
async function createOrganizationAIFunctionInstance(
    organizationID: number,
    data: SerializedAIFunctionInstanceWrite,
): Promise<SerializedAIFunctionInstance> {
    const { backendAPI } = config;
    let response = null;
    try {
        response = await Axios.post(
            `${backendAPI}/organizations/${organizationID}/ai-function-instances`,
            data,
        );
    } catch (errorData) {
        throw generateError(errorData);
...
async function updateOrganizationAIFunctionInstance(
    organizationID: number,
    slug: string,
    data: SerializedAIFunctionInstanceWrite,
): Promise<SerializedAIFunctionInstance> {
    ...
    try {
        response = await Axios.patch(
            `${backendAPI}/organizations/${organizationID}/ai-function-instances/${slug}`,
            data,
        );
    } catch (errorData) {
        throw generateError(errorData);
...
async function deleteOrganizationAIFunctionInstance(
    organizationID: number,
    slug: string,
): Promise<void> {
    const { backendAPI } = config;
    try {
        await Axios.delete(`${backendAPI}/organizations/${organizationID}/ai-function-instances/${slug}`);
    } catch (errorData) {
        throw generateError(errorData);
    }
}

async function enableOrganizationAIFunctionInstance(
    organizationID: number,
    slug: string,
): Promise<SerializedAIFunctionInstance> {
    const { backendAPI } = config;
    ...
    try {
        response = await Axios.post(
            `${backendAPI}/organizations/${organizationID}/ai-function-instances/${slug}/enable`,
        );
    } catch (errorData) {
        throw generateError(errorData);
    }
...
async function disableOrganizationAIFunctionInstance(
    organizationID: number,
    slug: string,
): Promise<SerializedAIFunctionInstance> {
    const { backendAPI } = config;
    ...
    try {
        response = await Axios.post(
            `${backendAPI}/organizations/${organizationID}/ai-function-instances/${slug}/disable`,
        );
    } catch (errorData) {
        throw generateError(errorData);
    }
...
async function setDefaultOrganizationAIFunctionInstance(
    organizationID: number,
    slug: string,
): Promise<SerializedAIFunctionInstance> {
    const { backendAPI } = config;
    ...
    try {
        response = await Axios.post(
            `${backendAPI}/organizations/${organizationID}/ai-function-instances/${slug}/set-default`,
        );
    } catch (errorData) {
        throw generateError(errorData);
    }
...
async function probeOrganizationHttpMicroservice(
    organizationID: number,
    data: {
        api_url: string;
    ...
    try {
        response = await Axios.post(
            `${backendAPI}/organizations/${organizationID}/ai-function-instances/probe`,
            data,
        );
    } catch (errorData) {
        throw generateError(errorData);
...
async function createOrganizationMembers(orgId: number, data: Record<string, any>) {
    const { backendAPI } = config;
    try {
        const response = await Axios.post(
            `${backendAPI}/organizations/${orgId}/create-members`,
            data,
        );
        return response.data;
    } catch (errorData) {
...
export default Object.freeze({
    ...
    events: Object.freeze({
        save: saveEvents,
        export: exportEvents,
        statistics: getAnnotationStatistics,
    }),
    ...
    organizations: Object.freeze({
        ...
        bailianSettings: Object.freeze({
            get: getOrganizationBailianSettings,
            update: updateOrganizationBailianSettings,
        }),
        aiFunctionInstances: Object.freeze({
            list: listOrganizationAIFunctionInstances,
            get: getOrganizationAIFunctionInstance,
            create: createOrganizationAIFunctionInstance,
            update: updateOrganizationAIFunctionInstance,
            delete: deleteOrganizationAIFunctionInstance,
            enable: enableOrganizationAIFunctionInstance,
            disable: disableOrganizationAIFunctionInstance,
            setDefault: setDefaultOrganizationAIFunctionInstance,
            probe: probeOrganizationHttpMicroservice,
        }),
        members: getOrganizationMembers,
        invitations: getMembershipInvitations,
        delete: deleteOrganization,
        invite: inviteOrganizationMembers,
        createMembers: createOrganizationMembers,
