...
export function validateAttributeValue(value: string, attr: Attribute): boolean {
    ...
    if (type === AttributeType.NUMBER) {
        // Blank means "unset" in the UI; do not block saving sibling attributes.
        if (value === '') {
            return true;
        }
    ...
    // select / radio: CVAT UI often leaves the field blank when default_value is
    // empty. Rejecting "" blocks editing any sibling attribute on the same object.
    if (value === '') {
        return true;
    }
