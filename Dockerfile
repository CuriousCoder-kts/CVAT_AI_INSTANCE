...
RUN for i in 1 2 3 4 5; do \
        echo "Downloading openh264 (attempt $i)..." && \
        curl -fsSL --retry 5 --retry-delay 3 --retry-max-time 300 \
            -o /tmp/openh264.tar.gz \
            https://github.com/cisco/openh264/archive/v${OPENH264_VERSION}.tar.gz && \
        tar -zx --strip-components=1 -f /tmp/openh264.tar.gz && \
        rm -f /tmp/openh264.tar.gz && break || \
        { echo "Attempt $i failed, retrying..."; rm -f /tmp/openh264.tar.gz; sleep 5; }; \
    done && \
...
RUN for i in 1 2 3 4 5; do \
        ...
        curl -fsSL --retry 5 --retry-delay 3 --retry-max-time 600 \
            -o /tmp/ffmpeg.tar.gz \
            https://ffmpeg.org/releases/ffmpeg-${FFMPEG_VERSION}.tar.gz && \
        tar -zx --strip-components=1 -f /tmp/ffmpeg.tar.gz && \
        rm -f /tmp/ffmpeg.tar.gz && break || \
        { echo "Attempt $i failed, retrying..."; rm -f /tmp/ffmpeg.tar.gz; sleep 10; }; \
    done && \
...
# Ensure shell entrypoint scripts have execute bits (defense against
# Windows / non-exec sources stripping the x-bit during COPY).
RUN chmod +x \
    /opt/cvat/backend_entrypoint.sh \
    /opt/cvat/wait_for_deps.sh && \
    find /opt/cvat/backend_entrypoint.d -type f -name "*.sh" -exec chmod +x {} \; 2>/dev/null || true
